...
{	
	...
	public float headRotOffset = 0f;
	public float headRotSensitivityMult = 1f;
	Vector3 startHeadRot;
    ...
    [Header("eSCOLA Vars")]//RB 
    public GameObject e_punRigTrackerPrefab;
    public bool mapFingerBones;
    [Header("Debug vars")]
    public bool debugRemoteTeacherLocal;
    public bool debugCreateLocalTeacher;
    public GameObject remoteTeacherPrefab;
...
//            jointPosition.y - offsetPos.y + kinectManager.sensorHeight,  //!! this should be better investigated .. 
    ...
    {	
        ...
        if (debugCreateLocalTeacher)
            Instantiate(remoteTeacherPrefab);
	}
    ...
    {	
		...
		// rotate the avatar bones // HEAD OFFSET
		for (var boneIndex = 0; boneIndex < bones.Length; boneIndex++)
		{
			...
			if(boneIndex2JointMap.ContainsKey(boneIndex))
			{
				...
				float offset = 0f;
				float sensitivityMult = 1f;

				if (joint == KinectInterop.JointType.Head)
                {
					offset = headRotOffset;
					sensitivityMult = headRotSensitivityMult;
				}
				...
				TransformBone(UserID, joint, boneIndex, !(mirroredMovement ^ flipLeftRight), offset, sensitivityMult);
			}
...
//		}
    ...
    protected void TransformBone(Int64 userId, KinectInterop.JointType joint, int boneIndex, bool flip, float offset, float sensitivityMult)
    {
		...
		Quaternion newRotation = Kinect2AvatarRot(jointRotation, boneIndex) * Quaternion.Euler(Vector3.right * offset);
...
//			return;
	...
	Vector3 offsetAdjust;

	public void ResetOffsetPos(Vector3 newOffset)
	{
		offsetAdjust = newOffset;
	}
	...
	protected void MoveAvatar(Int64 UserID)
	{
		...
		if(!offsetCalibrated)
		{
			...
			offsetPos.x = trans.x + offsetAdjust.x;  // !mirroredMovement ? trans.x * moveRate : -trans.x * moveRate;
			offsetPos.y = trans.y + offsetAdjust.y;  // trans.y * moveRate;
			offsetPos.z = !mirroredMovement && !posRelativeToCamera ? -trans.z : trans.z + offsetAdjust.z;  // -trans.z * moveRate;
...
//								(fNewDistance - fFootDistanceInitial), fFootDistance, fLastTgtY, targetPos.y, fFootDistanceTime));
    ...
    protected virtual void MapBones()
    {
        ...
        GameObject rigTracker = null;

        if (debugRemoteTeacherLocal)
            rigTracker = Instantiate(e_punRigTrackerPrefab);
        else 
            rigTracker = PhotonNetwork.Instantiate("PunRigTracker", Vector3.zero, Quaternion.identity, 0);
        ...
        for (int boneIndex = 0; boneIndex < bones.Length; boneIndex++)
		{
            ...
            {
                Debug.Log("Bone index " + boneIndex + " name " + bones[boneIndex].name + " mapped name " + boneIndex2MecanimMap[boneIndex].ToString());
                bones[boneIndex].name = boneIndex2MecanimMap[boneIndex].ToString(); // RB

                foreach (Transform t in rigTracker.transform)
                {
                    if (t.name == bones[boneIndex].name)
                    {
                        TransformFollow tFollow = t.GetComponent<TransformFollow>();
                        tFollow.otherToFollow = bones[boneIndex];
                        tFollow.boneIndex = boneIndex;
                    }
                }

                //GameObject boner = new GameObject(bones[boneIndex].name);
                //boner.AddComponent<TransformFollow>().otherToFollow = bones[boneIndex];
            }
        }
        ...
        if (mapFingerBones)
        {
            ...
            for (int boneIndex = 0; boneIndex < fingerBones.Length; boneIndex++)
            {
                ...
                if (fingerBones[boneIndex] != null)
                {
                    fingerBones[boneIndex].name = boneIndex2MecanimMap[boneIndex].ToString(); // RB

                    foreach (Transform t in rigTracker.transform)
                    {
                        if (t.name == fingerBones[boneIndex].name)
                            t.GetComponent<TransformFollow>().otherToFollow = fingerBones[boneIndex];
                    }
                }
            }
        }
...
//		{24, HumanBodyBones.RightToes},
	...
	public static readonly Dictionary<int, HumanBodyBones> fingerIndex2MecanimMap = new Dictionary<int, HumanBodyBones>
	{
